...
function AdminPage() {
  ...
  const linkClass = ({ isActive }) =>
    `flex items-center gap-3 px-4 py-2 rounded-lg transition-all ${
      isActive
        ? "bg-white/20 font-semibold backdrop-blur-md"
        : "hover:bg-white/10"
    }`;

  return (
  <>
   <div className="min-h-screen bg-fixed bg-cover bg-center relative" style={{ backgroundImage:`url(${adminWallpaper})` }}>
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 bg-white/10 backdrop-blur-lg border border-white/30 shadow-lg text-white flex flex-col`}>
        {/* Header */}
        <header className="flex items-center justify-between p-4 border-b border-white/20">
          <img src={logo} alt="Logo" className="ms-2 h-15 opacity-700"/>
        ...
        <footer className="p-4 border-t border-white/20">
          <div className="flex items-center gap-3 mb-3">
            <img src="https://images.unsplash.com/photo-1734122415415-88cb1d7d5dc0"className="w-8 h-8 rounded-full border border-white/30" alt="avatar"/>
            <span className="font-medium">Mia Hudson</span>
          </div>

          <div className="space-y-1 text-sm">
            <NavLink to="/admin/settings" className={linkClass}><FiSettings /> Settings </NavLink>
            <NavLink to="/admin/logout" className={linkClass}><FiLogOut /> Log out </NavLink>
          </div>
        </footer>
      </div>
